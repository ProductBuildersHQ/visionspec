...
## Synced Content (Generated — Do Not Edit)

Some template and rubric files under `pkg/workflows/default/` are **generated
copies** synced from [grokify/prism-roadmap](https://github.com/grokify/prism-roadmap),
the source of truth for roadmap/discovery/goals content. Affected families:
...
Each synced file carries a `Code generated by prism-sync; DO NOT EDIT`
provenance header naming its `prism-roadmap@<version>` source. **Never
hand-edit a synced file** — the next sync silently overwrites it. To change
one:

1. Change the upstream in prism-roadmap and cut a release.
2. Bump the require: `go get -C tools/prism-sync github.com/grokify/prism-roadmap@vX.Y.Z`
3. Re-sync: `go run -C tools/prism-sync .`
4. Verify: `go run -C tools/prism-sync . -check` and `go test ./...`

To change *which* files sync where, edit `tools/prism-sync/manifest.yaml`.
See `tools/prism-sync/README.md` for details. Locally-owned (non-synced)
templates/rubrics have no such header and are edited directly.
