...
- [ ] `RMI-SPECWORKFLOWSPEC-028` Reconcile the two PRD representations before any requirements-family migration (from RMI-025)
  - prism-roadmap models a PRD as a Go struct (`requirements/prd`: `PRDDocument`, programmatic completeness scoring); visionspec authors it as a markdown template + LLM rubric. Decide which is canonical (or how they compose — e.g. the Go model as the structured target the markdown/rubric drive toward) before moving prd/mrd/trd ownership. This is the real prerequisite RMI-025 identified; the ownership move stays deferred until it lands.
