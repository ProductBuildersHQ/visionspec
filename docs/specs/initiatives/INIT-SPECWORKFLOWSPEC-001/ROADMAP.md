...
- [ ] `RMI-SPECWORKFLOWSPEC-029` Backport N/A-with-rationale applicability to the lighter family rubrics' blocking categories (from RMI-026)
  - RMI-024 added layer metadata (class/blocking/evaluation) to the lighter rubrics but not applicability escape hatches. The RMI-026 calibration showed a blocking deterministic category (`security_considerations` in pbhq-lite's TRD rubric) false-negative-blocks a spec-content library where the category is genuinely N/A. Add `not_applicable` scale options (N/A-with-rationale, per the hardened enterprise/aws pattern, TRD-009c) to the blocking categories of the lighter family rubrics (pbhq-lite, quick-fix, startup, and peers) so gates don't misfire on non-service initiatives (libraries, tooling, migrations).
