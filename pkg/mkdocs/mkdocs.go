...
func GenerateProjectIndex(w io.Writer, report *status.Report, opts ProjectIndexOptions) error {
	...
	// Graph metrics and visualization if available
	if report.GraphMetrics != nil {
		...
		// Embed graph visualization if available
		if opts.IncludeGraphLink && opts.GraphPath != "" {
			if strings.HasSuffix(opts.GraphPath, ".html") {
				fmt.Fprintf(w, "### Visualization\n\n")
				fmt.Fprintf(w, "<iframe src=\"%s\" width=\"100%%\" height=\"500px\" frameborder=\"0\" style=\"border: 1px solid #ddd; border-radius: 4px;\"></iframe>\n\n",
					opts.GraphPath)
				fmt.Fprintf(w, "[Open full screen →](%s){target=_blank}\n\n", opts.GraphPath)
			} else {
				fmt.Fprintf(w, "[View Graph →](%s)\n\n", opts.GraphPath)
			}
		}
	}
...
// DetectGraphizeOutput looks for graphize visualization in a project.
// Returns the path to the HTML visualization if found, empty string otherwise.
func DetectGraphizeOutput(projectPath string) string {
	graphizeDir := filepath.Join(projectPath, ".graphize")

	// Check for HTML site export (graphize default output)
	htmlPath := filepath.Join(graphizeDir, "site", "index.html")
	if _, err := os.Stat(htmlPath); err == nil {
		return ".graphize/site/index.html"
	}

	// Check for standalone HTML file
	htmlPath = filepath.Join(graphizeDir, "graph.html")
	if _, err := os.Stat(htmlPath); err == nil {
		return ".graphize/graph.html"
	}

	// Check for GraphML (can be viewed with external tools)
	graphmlPath := filepath.Join(graphizeDir, "graph.graphml")
	if _, err := os.Stat(graphmlPath); err == nil {
		return ".graphize/graph.graphml"
	}
...
func GenerateProjectIndexWithGraph(projectPath string, report *status.Report) error {
	...
	// Detect graphize output
	graphPath := DetectGraphizeOutput(projectPath)
	if graphPath != "" {
		opts.IncludeGraphLink = true
		opts.GraphPath = graphPath
	}

	return WriteProjectIndex(projectPath, report, opts)
}
...
func EmbedGraphizeInIndex(w io.Writer, projectPath string, embedOpts GraphEmbedOptions) error {
	graphPath := DetectGraphizeOutput(projectPath)
	if graphPath == "" {
		return nil // No graph to embed
	}

	fmt.Fprintf(w, "\n## Requirement Graph\n\n")

	if embedOpts.Format == "" {
		embedOpts.Format = "iframe"
	}
	if embedOpts.Width == "" {
		embedOpts.Width = "100%"
	}
	if embedOpts.Height == "" {
		embedOpts.Height = "500px"
	}

	// For HTML output, use iframe
	if strings.HasSuffix(graphPath, ".html") {
		embedOpts.GraphURL = graphPath
		fmt.Fprintf(w, "<iframe src=\"%s\" width=\"%s\" height=\"%s\" frameborder=\"0\" style=\"border: 1px solid #ddd; border-radius: 4px;\"></iframe>\n\n",
			graphPath, embedOpts.Width, embedOpts.Height)
		fmt.Fprintf(w, "[Open in new tab](%s){target=_blank}\n\n", graphPath)
	} else {
		// For other formats, provide a download link
		fmt.Fprintf(w, "[Download Graph](%s)\n\n", graphPath)
	}
